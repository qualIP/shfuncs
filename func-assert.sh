...
assert_file_exists() {
    ...
    if ! [[ -e "$file" ]] ; then
        ...
        $loc_OPT_VERBOSE && print_dbg "$file: File or directory exists"
        return $EOK
    fi
}
...
assert_file_not_exists() {
    ...
    if [[ -e "$file" ]] ; then
        ...
        $loc_OPT_VERBOSE && print_dbg "$file: File or directory does not exist"
        return $EOK
    fi
}
...
assert_is_file() {
    ...
    if ! [[ -f "$file" ]] ; then
        if [[ -e "$file" ]] ; then
            ...
            print_err "$file: No such file"
            return $ENOENT
        fi
    else
        $loc_OPT_VERBOSE && print_dbg "$file: File exists"
        return $EOK
    fi
}
...
assert_is_directory() {
    ...
    if ! [[ -d "$file" ]] ; then
        if [[ -e "$file" ]] ; then
            ...
            print_err "$file: No such directory"
            return $ENOENT
        fi
    else
        $loc_OPT_VERBOSE && print_dbg "$file: Directory exists"
        return $EOK
    fi
}
